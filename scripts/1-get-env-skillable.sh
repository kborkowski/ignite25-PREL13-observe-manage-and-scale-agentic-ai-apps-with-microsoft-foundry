...
#              Fully automated for Skillable environment
# Usage: ./scripts/7-get-env-skillable.sh
# ============================================================================
...
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
echo -e "${BOLD}${CYAN}    Azure Environment Setup - Skillable${NC}"
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}\n"
...
if ! az account show &>/dev/null; then
    ...
    echo -e "${YELLOW}Please run: ${BLUE}az login${NC}"
    exit 1
fi
...
# ============================================================================
# Step 3: Auto-discover and select Resource Group
# ============================================================================
...
if [ "$IGNITE_RG_COUNT" -eq 0 ]; then
    echo -e "${RED}✗ No resource groups found with 'rg-Ignite' prefix${NC}"
    ...
    echo -e "\n${CYAN}Auto-selecting first resource group:${NC}"
    AZURE_RESOURCE_GROUP=$(echo "$IGNITE_RGS" | jq -r '.[0].name')
    AZURE_LOCATION=$(echo "$IGNITE_RGS" | jq -r '.[0].location')
    echo -e "  ${BOLD}→ $AZURE_RESOURCE_GROUP${NC} (${AZURE_LOCATION})"
fi
echo ""
...
if [ "$AOAI_COUNT" -gt 0 ]; then
    ...
    AZURE_AI_EMBED_DEPLOYMENT_NAME=$(echo "$DEPLOYMENTS" | jq -r '[.[] | select(.model | test("text-embedding-3-large"; "i"))] | .[0].name // ""')
    if [ -z "$AZURE_AI_EMBED_DEPLOYMENT_NAME" ]; then
        # Fallback to any embedding model if text-embedding-3-large not found
        AZURE_AI_EMBED_DEPLOYMENT_NAME=$(echo "$DEPLOYMENTS" | jq -r '[.[] | select(.model | test("embedding|embed"; "i"))] | .[0].name // ""')
    fi
    
    AZURE_AI_EMBED_MODEL_NAME=$(echo "$DEPLOYMENTS" | jq -r '[.[] | select(.name == "'"$AZURE_AI_EMBED_DEPLOYMENT_NAME"'")] | .[0].model // "text-embedding-3-large"')
    AZURE_AI_EMBED_MODEL_VERSION=$(echo "$DEPLOYMENTS" | jq -r '[.[] | select(.name == "'"$AZURE_AI_EMBED_DEPLOYMENT_NAME"'")] | .[0].version // "2"')
    AZURE_AI_EMBED_DEPLOYMENT_CAPACITY=$(echo "$DEPLOYMENTS" | jq -r '[.[] | select(.name == "'"$AZURE_AI_EMBED_DEPLOYMENT_NAME"'")] | .[0].capacity // "10"')
...
if [ -z "$AOAI_NAME" ]; then
    ...
    while IFS= read -r project; do
        if [[ "$project" == "$AOAI_NAME/"* ]]; then
            project_name="${project#$AOAI_NAME/}"
...
# ============================================================================
# Step 8: Configure Agent Name (Auto-select)
# ============================================================================
echo -e "${YELLOW}Step 8: Configuring Agent Name...${NC}"

AZURE_AI_AGENT_NAME="agent-template-assistant"
echo -e "${GREEN}Using agent name: $AZURE_AI_AGENT_NAME${NC}"
echo ""
# ============================================================================
...
if [ -n "$CONTAINERAPP_ENV" ]; then
    ...
    CONTAINER_APPS=$(az containerapp list \
        --resource-group "$AZURE_RESOURCE_GROUP" \
        --query "[].{name:name, fqdn:properties.configuration.ingress.fqdn, identityPrincipalId:identity.principalId}" \
        -o json 2>/dev/null || echo "[]")
    ...
    if [ "$APP_COUNT" -gt 0 ]; then
        ...
        SERVICE_FQDN=$(echo "$CONTAINER_APPS" | jq -r '.[0].fqdn // ""')
        SERVICE_API_IDENTITY_PRINCIPAL_ID=$(echo "$CONTAINER_APPS" | jq -r '.[0].identityPrincipalId // ""')
        
        if [ -n "$SERVICE_FQDN" ] && [ "$SERVICE_FQDN" != "null" ]; then
            ...
            SERVICE_API_URI=""
            echo -e "${YELLOW}⚠ Container App found but no FQDN configured${NC}"
        fi
    else
        SERVICE_API_NAME=""
        SERVICE_API_URI=""
...
# Auto-generated by scripts/7-get-env-skillable.sh on $(date)
# Resource Group: $AZURE_RESOURCE_GROUP
# ============================================================================
...
if [ -n "$AOAI_NAME" ]; then
    ...
    echo -e "  • Embedding Model: ${GREEN}${AZURE_AI_EMBED_DEPLOYMENT_NAME:-Not found}${NC} (${AZURE_AI_EMBED_MODEL_NAME})\n"
else
...
if [ -n "$SERVICE_API_NAME" ]; then
    echo -e "${BOLD}Container App:${NC} ${GREEN}$SERVICE_API_NAME${NC}\n"
else
    echo -e "${BOLD}Container App:${NC} ${YELLOW}Not found${NC}\n"
fi
